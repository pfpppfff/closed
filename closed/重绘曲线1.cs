...
namespace closed
{
    public partial class 重绘曲线1 : Form
    {
        public 重绘曲线1()
        {
            InitializeComponent();
            TestScaleCalculation(new double[] { 0f,38.89f }, "中等范围数据");
        }

        private void 重绘曲线1_Load(object sender, EventArgs e)
        {
            Curve2D1 cuv2D = new Curve2D1();
            cuv2D.Fit();
            Bitmap bitmap = cuv2D.CreateImage();
            this.pictureBox1.Image = cuv2D.CreateImage();
            cuv2D.CreateImage().Save(@"D://image自定义分辨率.bmp");
            ...
            //    var image = worksheet.AddPicture(memoryStream, "GeneratedImage").MoveTo(worksheet.Cell("B2"), worksheet.Cell("L19"));
            //    // 保存Excel文件
            ...
            //}
            string filePath = "output_with_image.xlsx";
            string filePath1 = "重绘制曲线1.xlsx";
            // 创建一个新的Excel包，如果文件存在则加载它
            using (var package = new ExcelPackage(new FileInfo(filePath)))
            {
                // 获取第一个工作表，如果没有则添加一个新的
                ExcelWorksheet worksheet = package.Workbook.Worksheets.FirstOrDefault();
                if (worksheet == null)
                {
                    worksheet = package.Workbook.Worksheets.Add("Sheet1");
                }                                         
                var picture = worksheet.Drawings.AddPicture("pictureName1", bitmap);
                picture.SetPosition(40, 0, 2, 0); // 设置图片左上角相对于A1单元格的位置
                picture.SetSize(bitmap.Width-400, bitmap.Height-200); // 设置图片的尺寸
               
                package.SaveAs(new FileInfo(filePath1));
            }
        }

        static void TestScaleCalculation(double[] datas, string description)
        {
             
            try
            {
                var scale = AdvancedScaleCalculator.CalculateOptimalScale(datas);
                Console.WriteLine(scale);
                
                Console.WriteLine("刻度值:");
                double[] ticks = scale.GenerateTicks();

                Console.WriteLine($"最小值{scale.Start} 最大值{scale.End} 间隔{scale.Step} 刻度数{scale.TickCount}" );
            }
            catch (Exception ex)
            {
        ...
        {
            TestScaleCalculation(new double[] { 0f,6f}, "中等范围数据");
        }
    ...
    public class Curve2D1
    {
        private Graphics objGraphics; //Graphics 类提供将对象绘制到显示设备的方法
        private Bitmap objBitmap; //位图对象

        private float fltWidth =1000; //图像宽度
        private float fltHeight = 600; //图像高度
        private float resolutionW =300;
        private float resolutionH =300;
        private float fltXSlice = 40; //X轴刻度单边长度
        private float fltYSlice = 40; //Y轴刻度单边长度
        ...
        private float fltXSpaceRightOffSet = 30f; //图像右距离边缘距离偏移值 相对于fltXSpace的偏移
        private float fltXSpaceLeftOffSet = 75f; //图像左距离边缘距离偏移值 Y3轴使用
        private float fltXSpace = 120f; //图像左右距离边缘距离
        private float fltYSpace = 10f; //图像上下距离边缘距离
        private float intFontSize = 4f; //字体大小号数
        private float fltXRotateAngle = 0f; //X轴文字旋转角度
        private float fltYRotateAngle = 0f; //Y轴文字旋转角度
        private int intCurveSize = 3; //曲线线条大小
        private float intAxisLineSize = 2.0f; //坐标线条宽度
        private float intFontSpace = 0; //intFontSpace 是字体大小和距离调整出来的一个比较适合的数字
        private float[] y_XOffSet = { -15f, 5f, -5f ,-5f};//x刻度值置位偏移
        private float[] y_YOffSet = { -5f, -5f, -5f ,5f};//y刻度值置位偏移
        ...
        {
            get { return intFontSize* (ResolutionW /96) * (intFontSize / 11); }
        ...
        /// 坐标线条大小
        /// </summary>
        public float AxisLineSize
        {
            get { return intAxisLineSize; }
            set { intAxisLineSize = value; }
        }
        #endregion
        ...
        public void Fit()
        {
            ...
            //计算图像边距
            float fltSpace = Math.Min(Width /8, Height / 8);
        ...
        private void InitializeGraph()
        {
            ...
            objGraphics.DrawRectangle(new Pen(Color.Transparent, 1), 0, 0, Width - 1, Height - 1); //画边框
            ...
            float fltY2 = fltY1;
            objGraphics.DrawLine(new Pen(new SolidBrush(AxisColor), AxisLineSize), fltX1, fltY1, fltX2, fltY2);
        ...
        private void SetAxisText(ref Graphics objGraphics)
        {
            ...
            float fltX = Width - XSpace + XSpaceRightOffSet - 2 * FontSize * XAxisText.Length;
            float fltY = Height - YSpace + FontSize +30;
            objGraphics.DrawString(XAxisText, fontStr, new SolidBrush(AxisTextColor[0]), fltX, fltY);
            ...
            fltX = XSpace - YSlice / 10 * 1.5f - FontSize* YAxisText[0].Length -35;
            fltY = YSpace + sizeF1.Width +40;
            ...
            fltX = Width - XSpace + XSpaceRightOffSet + YSlice / 10 * 1.5f + sizeF.Width +32;
            fltY = YSpace + sizeF1.Width +80;
            objGraphics.TranslateTransform(fltX, fltY);
            objGraphics.RotateTransform(270);
            ...
            fltX = XSpace - XSpaceRightOffSet /*/*+ YSlice / 10 * 1.5f*/ - sizeF.Height-20;
            fltY = Height - YSpace;
            objGraphics.TranslateTransform(fltX, fltY);
            objGraphics.RotateTransform(270);
        ...
        private void SetXAxis(ref Graphics objGraphics)
        {
            ...
            objGraphics.DrawString(XSliceBegin.ToString(), font, new SolidBrush(SliceTextColor), fltX1 - (int)font.GetHeight() / 2, fltY1+(int)font.GetHeight() / 2);
            ...
            for (int i = 0; i <= XSpaceNum * 5; i++)
            {
                ...
                if (iCount == 5)
                {
                    ...
                    //这里显示X轴刻度字符串
                    string strX = (XSliceBegin + xPerLen * (iSliceCount)).ToString();
                    objGraphics.DrawString((XSliceBegin + xPerLen * (iSliceCount)).ToString(), font, new SolidBrush(SliceTextColor), fltX1 + Scale -  FontSize * strX.ToString().Length+ y_XOffSet[3], fltY1+(int)font.GetHeight() + y_YOffSet[3]);
        ...
        private void SetY1Axis(ref Graphics objGraphics)
        {
            ...
            strSliceText = "0";
            objGraphics.DrawString(strSliceText, font, new SolidBrush(SliceTextColor), XSpace - 2 * FontSize * strSliceText.Length+ y_XOffSet[0], Height - YSpace - (int)font.GetHeight() + y_YOffSet[0]);
            ...
            for (int i = 0; i <= YSpaceNum[0] * 5; i++)
            {
                ...
                if (iCount == 5)
                {
                    ...
                    objGraphics.DrawString((YSliceBegin[0] + yPerLen * (iSliceCount)).ToString(), font, new SolidBrush(SliceTextColor), XSpace - 2*FontSize * strSliceText.Length + y_XOffSet[0], fltY1 - Scale + higtOffset - (int)font.GetHeight() / 2 + y_YOffSet[0]);
        ...
        private void SetY2Axis(ref Graphics objGraphics)
        {
            ...
            strSliceText = "0";
            //0刻度         
            objGraphics.DrawString(strSliceText, font, new SolidBrush(SliceTextColor), Width - XSpace + XSpaceRightOffSet + y_XOffSet[1], Height - YSpace - (int)font.GetHeight()/2 + y_YOffSet[1]);         
            for (int i = 0; i <= YSpaceNum[1] * 5; i++)
            {
                ...
                if (iCount == 5)
                {
                    ...
                    objGraphics.DrawString((YSliceBegin[1] + yPerLen * (iSliceCount)).ToString(), font, new SolidBrush(SliceTextColor), Width - XSpace + XSpaceRightOffSet + y_XOffSet[1], fltY1 - Scale - (int)font.GetHeight()/2 + y_YOffSet[1]);
        ...
        private void SetY3Axis(ref Graphics objGraphics)
        {
            ...
            strSliceText = "0";
            //0点刻度值
            objGraphics.DrawString(strSliceText, font, new SolidBrush(SliceTextColor), fltX1 - 2*FontSize * strSliceText.ToString().Length- fltSliceWidth * 1.5f + y_XOffSet[2], Height - YSpace - (int)font.GetHeight() + y_YOffSet[2]);
            ...
            for (int i = 0; i <= YSpaceNum[2] * 5; i++)
            {
                ...
                if (iCount == 5)
                {
                    ...
                    int higtOffset = -(int)font.GetHeight();
                    objGraphics.DrawString((YSliceBegin[0] + yPerLen * (iSliceCount)).ToString(), font, new SolidBrush(SliceTextColor), fltX1 -2* FontSize * strSliceText.Length + y_XOffSet[2]- fltSliceWidth * 1.5f, fltY1 - Scale+ higtOffset+y_YOffSet[2]);
        ...
        {
            float cintFontSize = FontSize + 4;
            objGraphics.DrawString(Title, new Font("楷体", cintFontSize, FontStyle.Bold), new SolidBrush(TextColor), new Point((int)(0.5 * Width - 0.5 * cintFontSize * (Title.Length + 4))-50, (int)(YSlice / 2 + intFontSpace)-10));
        }
    }


    //坐标刻度优化算法
    public class AdvancedScaleCalculator
    {
        /// <summary>
        /// 计算最佳坐标刻度
        /// </summary>
        /// <param name="minValue">输入数据的最小值</param>
        /// <param name="maxValue">输入数据的最大值</param>
        /// <returns>坐标刻度信息</returns>
        public static ScaleInfo CalculateOptimalScale(double[] data)
        {
            ...
            // 定义可接受的刻度间隔
            double[] niceNumbers = { 1, 2, 5, 10, 20, 25, 50, 100, 200, 250, 500, 1000 };

            // 存储最佳结果的变量
            double bestTickStep = 0;
            int bestTickCount = 0;
            double bestScaleMin = 0;
            double bestScaleMax = 0;

            // 遍历不同的刻度间隔，找到最佳方案
            foreach (double baseStep in niceNumbers)
            {
                // 尝试不同的放大系数
                double[] magnitudes = { 0.1, 1, 10, 100, 1000 };
                foreach (double magnitude in magnitudes)
                {
                    double tickStep = baseStep * magnitude;

                    // 计算起始和结束刻度
                    double scaleMin = Math.Floor(minValue / tickStep) * tickStep;
                    double scaleMax = Math.Ceiling(maxValue / tickStep) * tickStep;

                    // 计算刻度数
                    int tickCount = (int)Math.Round((scaleMax - scaleMin) / tickStep);

                    // 检查刻度数是否在理想范围内(5-10)
                    if (tickCount >= 5 && tickCount <= 10)
                    {
                        // 如果这是第一个找到的方案，或者比之前的方案更好
                        if (bestTickCount == 0 ||
                            Math.Abs(tickCount - 7.5) < Math.Abs(bestTickCount - 7.5) ||
                            (tickCount == bestTickCount && tickStep < bestTickStep))
                        {
                            bestTickStep = tickStep;
                            bestTickCount = tickCount;
                            bestScaleMin = scaleMin;
                            bestScaleMax = scaleMax;
                        }
                    }
                }
            }
            ...
            if (bestTickCount == 0)
            {
                throw new InvalidOperationException("无法找到合适的坐标刻度");
            }

            return new ScaleInfo(bestScaleMin, bestScaleMax, bestTickStep,bestTickCount);
        }
    }
    ...
    {
        public double Start { get; }      // 起始刻度值
        public double End { get; }        // 结束刻度值
        public double Step { get; }       // 刻度间隔
        public int TickCount { get; }     // 刻度数量

        public ScaleInfo(double start, double end, double step , int tickCount)
        {
            Start = start;
            End = end;
            Step = step;
            TickCount = (int)Math.Round((end - start) / step);
        }

        /// <summary>
        /// 生成刻度值数组
        /// </summary>
        public double[] GenerateTicks()
        {
            List<double> ticks = new List<double>();
            for (double tick = Start; tick <= End; tick += Step)
            {
                ticks.Add(Math.Round(tick, 10)); // 防止浮点数精度问题
            }
        ...
        {
            return $"起始值: {Start}, 结束值: {End}, 刻度间隔: {Step}, 刻度数: {TickCount}";
        }
    }
}
