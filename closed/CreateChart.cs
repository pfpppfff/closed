...
namespace closed
{
    ...
    class CreateChart
    {
        ...
        //    int marginTop = 60;
        //    int marginBottom = 60;
        //    int marginSecondLeftAxis = 50;
        //}

        public static Bitmap CreatePumpPerformanceChart(double[] flow, double[] head, double[] efficiency, double[] power)
        {         
            // 图表尺寸和边距
            int width = 900;
            int height = 550;
            ...
            // 计算X轴范围
            double xMin = 0;
            double xMax = flow.Max() * 1.1; // 轴线留一些额外空间
            //double xCurveMax = flow.Max() * 1.2;
            // 计算Y轴范围
            double headMin = 0;
            ...
            // 绘制左侧Y轴(扬程-红)
            DrawLeftAxis(g, headMin, headMax, marginLeft, marginTop, chartHeight, "Head (m)", Color.Red, Xaisfontsize);

            // 绘制左侧第二Y轴(功率-绿) - 修改为扬程Y轴高度的一半，从底部对齐
            DrawSecondLeftAxis(g, powerMin, powerMax, marginLeft - marginSecondLeftAxis, marginTop + chartHeight / 2, chartHeight / 2, "Power (kW)", Color.Green, Xaisfontsize);

            // 绘制右侧Y轴(效率-蓝)
            DrawRightAxis(g, efficiencyMin, efficiencyMax, marginLeft + chartWidth, marginTop, chartHeight, "Efficiency (%)", Color.Blue, Xaisfontsize);
        ...
        static void DrawLeftAxis(Graphics g, double min, double max, int x, int y, int height, string label, Color color, Font AxisLablefont)
        {
            Font axisFont = AxisLablefont;
            using (Pen axisPen = new Pen(color, 1))
            using (StringFormat format = new StringFormat())
            {
                ...
                for (int i = 0; i <= tickCount; i++)
                {
                    ...
                    // 绘制刻度线
                    g.DrawLine(axisPen, x - 5, tickY, x, tickY);
        ...
        static void DrawSecondLeftAxis(Graphics g, double min, double max, int x, int y, int height, string label, Color color, Font AxisLablefont)
        {
            Font axisFont = AxisLablefont;
            using (Pen axisPen = new Pen(color, 1))
            using (StringFormat format = new StringFormat())
        ...
        static void DrawRightAxis(Graphics g, double min, double max, int x, int y, int height, string label, Color color, Font AxisLablefont)
        {
            Font axisFont = AxisLablefont;
            using (Pen axisPen = new Pen(color, 1))
            using (StringFormat format = new StringFormat())
            {
                ...
                for (int i = 0; i <= tickCount; i++)
                {
                    ...
                    // 绘制刻度线
                    g.DrawLine(axisPen, x, tickY, x + 5, tickY);
