...
namespace UserDefinedControl.OPCUA
{
    ...
    public sealed class AsyncOpcUaService : IOpcUaService
    {
        ...
        private async Task<T> ExecuteWithRetryAsync<T>(Func<Task<T>> operation, T defaultValue)
        {
            try
            {
                ...
                using (var cts = new CancellationTokenSource(OperationTimeout))
                {
                    // 创建任务包装器以支持超时
                    var task = operation();
                    var timeoutTask = Task.Delay(OperationTimeout, cts.Token);
            ...
            catch (Exception ex)
            {
                ...
                await Task.Delay(100); // 短暂延迟避免忙等待
                return defaultValue;
            }
        }
        ...
        /// 带重试机制和取消令牌支持的异步执行
        /// </summary>
        private async Task<T> ExecuteWithRetryAsync<T>(Func<CancellationToken, Task<T>> operation, T defaultValue, CancellationToken cancellationToken = default)
        {
            try
            {
                // 创建带超时的取消令牌源，与传入的令牌链接
            ...
            catch (Exception ex)
            {
                ...
                await Task.Delay(100, cancellationToken); // 短暂延迟避免忙等待
                return defaultValue;
            }
        }
