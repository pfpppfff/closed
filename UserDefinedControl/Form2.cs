...
namespace UserDefinedControl
{
    ...
    public partial class Form2 : Form
    {
        ...
        //                await Task.Delay(1, _cancellationTokenSource.Token); // 500ms 更新间隔
        //            }
            ...
            await Task.Run(async () =>
            {
                while (_isMonitoring && !_cancellationTokenSource.Token.IsCancellationRequested)
                {
                    ...
                    try
                    {
                        ...
                        if (elapsedMs >= 20)
                        {
                            LogMessage($"⚠️ OPC 数据采集超时！耗时: {elapsedMs} ms (阈值: 100 ms)");
                        }
                        else
                        {
                            // 可选：只在调试时记录正常耗时，避免日志刷屏
                            // LogMessage($"OPC 数据采集完成，耗时: {elapsedMs} ms");
                        }

                        // 采集间隔：如果你希望每 100ms 采集一次，这里用 100
                        // 如果希望“尽快下一次”，且采集本身 <100ms，也可以用更短 delay 或无 delay
                        await Task.Delay(2, _cancellationTokenSource.Token); // 例如：10ms 间隔，确保高频但不过载
                    }
                    catch (OperationCanceledException)
                    {
                        break;
                    ...
                    catch (Exception ex)
                    {
                        stopwatch.Stop();
                        var elapsedMs = stopwatch.ElapsedMilliseconds;
                        LogMessage($"❌ 数据读取失败（耗时 {elapsedMs} ms）: {ex.Message}");
                        await Task.Delay(1000, _cancellationTokenSource.Token); // 错误后降频
                    }
                }
            }, _cancellationTokenSource.Token);
        }
